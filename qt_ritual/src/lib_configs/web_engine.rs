...
pub fn web_engine_core_config(config: &mut Config) -> Result<()> {
    let namespace = CppPath::from_good_str("QtWebEngineCore");
    config.set_rust_path_scope_hook(move |path| {
        if path == &namespace {
            return Ok(Some(RustPathScope {
                path: RustPath::from_good_str("qt_web_engine_core"),
                prefix: None,
            }));
        }
        Ok(None)
